...
import { MOCK_COMPETITIONS, CompetitionStatus } from '@/data/mockCompetitions';
import { CompetitionStatusBadge } from '@/components/dashboard/competitions/cards/CompetitionStatusBadge';
import { Search, Filter, Trophy, Plus, ArrowRight, ChevronDown, ChevronUp } from 'lucide-react';
...
const columnHelper = createColumnHelper<typeof MOCK_COMPETITIONS[0]>();

const columns = [
  columnHelper.accessor('code', {
    header: 'Code',
    cell: info => <span className="font-bold text-gray-500 text-[10px] uppercase tracking-wider">{info.getValue()}</span>,
  }),
  columnHelper.accessor('name', {
    header: 'Competition Name',
    cell: info => (
      <div>
        <Link to={`/dashboard/competitions/${info.row.original.id}`} className="font-bold text-gray-900 dark:text-white hover:text-[#0098c8] hover:underline">
          {info.getValue()}
        </Link>
  ...
  columnHelper.accessor('prizePool', {
    header: 'Prize Pool',
    cell: info => <span className="font-black text-amber-600 dark:text-amber-500">${info.getValue().toLocaleString()}</span>
  }),
  ...
  columnHelper.accessor('status', {
    header: 'Status',
    cell: info => <CompetitionStatusBadge status={info.getValue()} />
  }),
  columnHelper.display({
    ...
    cell: info => (
      <div className="flex items-center gap-2">
        <Link 
          to={`/dashboard/competitions/${info.row.original.id}`}
...
export const CompetitionList = () => {
  ...
  const filteredData = MOCK_COMPETITIONS.filter(comp => {
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Browse, filter, and manage all university events and hackathons.</p>
        </div>
        <Link to="/dashboard/competitions/new" className="px-4 py-2 bg-gradient-to-r from-[#0098c8] to-blue-600 text-white rounded-lg text-sm font-bold hover:opacity-90 transition-opacity shadow-sm flex items-center">
          <Plus size={16} className="mr-2" /> New Competition
        </Link>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 p-4 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex items-center gap-4 overflow-x-auto pb-2 lg:pb-0 scrollbar-hide shrink-0">
          ...
          <button className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm flex items-center">
            <Filter size={16} className="mr-2 text-gray-400" /> Advanced
          </button>
        </div>
      </div>
              ...
              {filteredData.length === 0 && (
                <tr>
                  <td colSpan={columns.length} className="p-12 text-center text-gray-500">
                    <Trophy size={48} className="mx-auto mb-4 text-gray-300 dark:text-gray-700" />
                    <p className="text-lg font-bold text-gray-900 dark:text-white">No competitions found</p>
